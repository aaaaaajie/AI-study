...
function keywordRetrieveWithScores(question: string) {
	...
	return { hits: scored, ms: Date.now() - started };
}
...
async function hybridRetrieve(question: string) {
	...
	const hybridMs = semantic.ms + keyword.ms;
	return {
		selected,
		semantic,
		keyword,
...
async function answer(question: string): Promise<AnswerResult> {
	const started = Date.now();
	await initKnowledgeBase();

	const retrieveStart = Date.now();
	const retrieved = await hybridRetrieve(question);
	...
	// 关键术语覆盖校验：上下文若不包含问题中的关键术语，则视为不可回答
	const questionTerms = extractKeywords(question).slice(0, 8);
	const lowerContext = context.toLowerCase();
	const coveredTerms = questionTerms.filter((t) => lowerContext.includes(t.toLowerCase()));
...
${question}
	...
	const logPayload = {
		...
		timing: {
			retrieveMs,
			semanticMs: retrieved.semantic.ms,
			keywordMs: retrieved.keyword.ms,
			llmMs,
			totalMs,
		},
		termCoverage: {
			...
			ratio: questionTerms.length ? coveredTerms.length / questionTerms.length : 0,
		},
		cost: {
			embeddingQueries: 1, // 只对 query 做一次 embedding；文档 embedding 已缓存
			llmCalls: fallback ? 0 : 1,
		},
		candidates: {
			semantic: retrieved.semantic.candidates.map((c) => ({
				score: c.score,
				docId: (c.doc.metadata as any)?.docId,
				chunkIndex: (c.doc.metadata as any)?.chunkIndex,
			})),
			passed: retrieved.semantic.passed.map((c) => ({
				score: c.score,
				docId: (c.doc.metadata as any)?.docId,
				chunkIndex: (c.doc.metadata as any)?.chunkIndex,
			})),
			keyword: retrieved.keyword.hits.map((c) => ({
				score: c.score,
				docId: (c.doc.metadata as any)?.docId,
				chunkIndex: (c.doc.metadata as any)?.chunkIndex,
			})),
			hybridTop: retrieved.mergedList
		...
		finalDocs: docs.map((d) => ({
			docId: (d.metadata as any)?.docId,
			chunkIndex: (d.metadata as any)?.chunkIndex,
			len: (d.metadata as any)?.chunkLen,
		})),
		fallback,
	};

	console.log("[RAG][query]", question);
	console.log("[RAG][timing]", logPayload.timing);
	console.log("[RAG][cost]", logPayload.cost);
	console.log("[RAG][term_coverage]", logPayload.termCoverage);
	console.log("[RAG][candidates]", JSON.stringify(logPayload.candidates, null, 2));
	console.log("[RAG][final_docs]", logPayload.finalDocs);
...
function startServer() {
	...
	app.listen(PORT, () => {
		console.log(`[RAG] 服务已启动：http://localhost:${PORT}/rag/answer`);
	});
}
